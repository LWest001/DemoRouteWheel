            ...
            for (rp = 0; rp < n_repetition; rp++) {
                ...
                for (backCount = 20; backCount > 0; backCount--) {
                    if (target_num - backCount + 1 >= 0) {
                        targetSeries.push(img_host + "ISC1/" + ('00000' + (target_num - backCount + 1)).slice(-6) + '.jpg')
                    } else {
                        targetSeries.push(img_host + "ISC1/" + ('00000' + (target_num - backCount + 361)).slice(-6) + '.jpg')
                    }
                }
                basic_condition.push({
                    radius: radius[r],
                    scene_num: ('00000' + (bin_startPoint[b] + shuffled_jitter[rp])).slice(-6).toString(),
                    wheel_path: img_host + "ISC1/", //Wheel path is the whole folder, so it the image can change base on the mouse, by Bill 11/23/2021 // change to IS2 by Leo 12/2/2021
                    //wheel_path: img_host+'Wheel'+wheel_num+'/wheel'+wheel_num+'_r'+radius[r]+'/', //Wheel path 
                    img_path: img_host + "ISC1/" + ('00000' + (bin_startPoint[b] + shuffled_jitter[rp])).slice(-6) + '.jpg', // img_path is the image for the target, change it to ISC, by Bill 11/23/2021// change to IS2 by Leo 12/2/2021
